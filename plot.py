...
df = pd.read_csv("waves_filtered.csv")
...
plt.title('Atlantic Ocean Average Wave Length Distribution')
...
plt.text(0.9, 0.9, "Most Frequent Bins (ratio ~ 0.5):", fontsize=10, transform=plt.gca().transAxes, ha='right')
for i, (bin_val, bin_freq) in enumerate(top_bins):
...
freq_bins_101 = [(bin_val, bin_freq) for bin_val, bin_freq in zip(bins, hist) if 100.5 < bin_val < 101.5]

# Annotate the plot with the frequency of all bins starting with 101 --> 101.25 is 0.75 * 135 where 135 is ship length
plt.text(0.65, 0.9, "Most Frequent Bins close to 0.75:", fontsize=10, transform=plt.gca().transAxes, ha='right')
for i, (bin_val, bin_freq) in enumerate(freq_bins_101):
...
freq_bins_84 = [(bin_val, bin_freq) for bin_val, bin_freq in zip(bins, hist) if 83.5 < bin_val < 84.5]

# Annotate the plot with the frequency of all bins starting with 101
plt.text(0.65, 0.6, "Most Frequent Bins close to 0.625:", fontsize=10, transform=plt.gca().transAxes, ha='right')
for i, (bin_val, bin_freq) in enumerate(freq_bins_84):
    plt.text(0.65, 0.55 - i * 0.05, f"Bin Value: {bin_val:.2f}, Frequency: {bin_freq}", fontsize=8,
             transform=plt.gca().transAxes, ha='right')
